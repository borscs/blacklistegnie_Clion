...
bool Database::findInDatabase(const QString &md5, const QString &sha1, const QString &sha256)
{
	QSqlQuery sqlQuery;
	sqlQuery.prepare(
		"SELECT md5, sha1, sha256 FROM hashTable WHERE md5 = (:md5Var) OR sha1 = (:sha1Var) OR sha256 = (:sha256Var)");
...
bool Database::findInDatabase(const QString &hash) // call the param to hash
{
	...
	switch (hash.length()) {
		case 32:sqlQuery.prepare("SELECT md5 FROM hashTable WHERE md5 = (:md5Var)");
			sqlQuery.bindValue(":md5Var", hash);
			break;
		case 40:sqlQuery.prepare("SELECT sha1 FROM hashTable WHERE sha1 = (:sha1Var)");
			sqlQuery.bindValue(":sha1Var", hash);
			break;
		case 64:sqlQuery.prepare("SELECT sha256 FROM hashTable WHERE sha256 = (:sha256Var)");
